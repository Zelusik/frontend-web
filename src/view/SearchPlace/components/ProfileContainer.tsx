...
const ProfileContainer = ({ currentIndex, keyword }: any) => {
  const infinityScrollRef = useRef<any>(null);
  const { display } = useAppSelector((state) => state.global);

  const { searchDatas, isLoading, fetchNextPage, hasNextPage } =
    useGetSearchProfile(currentIndex === 2, currentIndex, keyword);
        ...
        <>
          {searchDatas
            ...
            ?.map((data: any, idx: number) => {
              return (
                <ProfileSelection key={idx} data={data} keyword={keyword} />
              );
            })}
